...
const renderStars = (rating: number) => {
    ...
    const stars = [];
    for (let i = 0; i < fullStars; i++) stars.push(<svg key={`full-${i}`} className='w-5 h-5 text-warning' aria-hidden='true'><use xlinkHref='#star-full'></use></svg>);
    if (halfStar) stars.push(<svg key="half" className='w-5 h-5 text-warning' aria-hidden='true'><use xlinkHref='#star-half'></use></svg>);
    return stars;
};
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-start">
      ...
      <div className="product-details">
        <Link href={`/category/${product.category}`} className="text-sm text-primary hover:underline mb-2 inline-block capitalize">
          {product.category.replace('-', ' & ')}
        </Link>
        <h1 className="text-3xl md:text-4xl font-bold text-dark mb-3">{product.title}</h1>
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          >
            <svg className='w-5 h-5 mr-2' aria-hidden='true'><use xlinkHref='#cart'></use></svg>
            Add to Cart
          </button>
        </div>
        ...
        >
          <svg className='w-5 h-5' aria-hidden='true'><use xlinkHref='#heart'></use></svg>
